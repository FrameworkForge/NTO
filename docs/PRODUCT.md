...
NTO is intended to become an open-source photography platform taking photographers from camera to customer: Shoot → Import → Cull → Edit → Publish → Sell → Deliver. This repository holds the whole ecosystem. It is proposed to the maintainer that it stay private and that Studio and the shared contracts be published in a separate open-source repository, with the web integration and Cloud staying private. Current code is not open-source licensed merely because that is the intended direction; no LICENSE file exists until the owner adopts one.
