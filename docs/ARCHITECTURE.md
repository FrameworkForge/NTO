...
The app target's `WorkspaceView` composes a `NavigationSplitView` (sidebar, content, inspector) over the same three objects as before: `WorkspaceState`, `ProjectStore` and `LibraryController`. Mode content is `PhotoLibraryGrid`, `SelectedPhotoCanvas` (Cull and Edit) or `PublishWorkspace`; the inspector is `EditInspector` in Edit and `PhotoMetadataInspector` otherwise. `LibraryShowFilter` maps the Library's Show pop-up onto the existing `LibraryQuery` fields without touching search or sort. Shared controls (`RatingStars`, `FlagSegment`, `FloatingBar`, `PixelInspection`) live in `Views/Controls.swift`. Design mapping and deliberate deviations are recorded in [STUDIO-NATIVE-UI.md](STUDIO-NATIVE-UI.md).
